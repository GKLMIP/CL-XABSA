...
def build_or_load_dataset(args, tokenizer, mode='train'):
    ...
    args.task = 'absa'
    # args.task
    _, tag2idx, _ = get_tag_vocab(args.task, tagging_schema=args.tagging_schema,
    ...
    if mode == 'dev':
        ...
        elif exp_type == 'teacher_one':
            file_name_or_list = [f"gold-{args.src_lang}-train.txt",
                                 f"smt-{args.tgt_lang}-train.txt"]


        elif exp_type == 'teacher_two':
            file_name_or_list = [f"smt-{args.tgt_lang}-train.txt",
                                 f"cs_{args.src_lang}-{args.tgt_lang}-train.txt"]


        elif exp_type == 'teacher_three':
            file_name_or_list = [f"smt-{args.tgt_lang}-train.txt",
                                 f"cs_{args.tgt_lang}-{args.src_lang}-train.txt"]
    ...
    elif mode == 'translate_original':
        file_name_or_list = f"gold-{args.src_lang}-train.txt"

    elif mode == 'translate_finetune':
        file_name_or_list = f"smt-{args.tgt_lang}-train.txt"
