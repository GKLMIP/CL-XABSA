...
def init_args():
    ...
    output_dir = f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-{args.exp_type}-{args.seed}-unsupervised"
...
def train(args, train_dataset, train_dataset_original, train_dataset_finetune, model, tokenizer):
    """ Train the model """
    print(train_dataset_finetune[0])
    ...
    train_finetune_iterator = 5
    t_total = len(train_dataset_finetune) // args.gradient_accumulation_steps * train_finetune_iterator
    optimizer_grouped_parameters_finetune = get_optimizer_grouped_parameters(args, model, no_grad=no_grad)
    optimizer_finetune = AdamW(optimizer_grouped_parameters_finetune, lr=args.learning_rate, eps=args.adam_epsilon)
    # scheduler = WarmupLinearSchedule(optimizer, warmup_steps=args.warmup_steps, t_total=t_total)
    scheduler = get_linear_schedule_with_warmup(optimizer_finetune, num_warmup_steps=args.warmup_steps, num_training_steps=t_total)

    for n_finetune_epoch in range(train_finetune_iterator):
        epoch_train_loss = 0.0
        batch_size_finetune = 32
        print(len(train_dataset_finetune))
        print(len(train_dataset_original))
        iterator = int(len(train_dataset_finetune) / batch_size_finetune) + 1
        for iterator_num in range(iterator):
            if iterator_num*batch_size_finetune+batch_size_finetune < len(train_dataset_finetune):
                batch = train_dataset_original[
                        iterator_num*batch_size_finetune:iterator_num*batch_size_finetune+batch_size_finetune]
                batch2 = train_dataset_finetune[
                         iterator_num*batch_size_finetune:iterator_num*batch_size_finetune+batch_size_finetune]
            else:
                batch = train_dataset_original[
                        iterator_num * batch_size_finetune:len(train_dataset_finetune)]
                batch2 = train_dataset_finetune[
                         iterator_num * batch_size_finetune:len(train_dataset_finetune)]


            batch['input_ids'] = torch.cat((batch['input_ids'],batch2['input_ids']),1)
            batch['token_type_ids'] = torch.cat((batch['token_type_ids'],batch2['token_type_ids']),1)
            batch['attention_mask'] = torch.cat((batch['attention_mask'],batch2['attention_mask']),1)
            batch['labels'] = torch.cat((batch['labels'],batch2['labels']),1)
            ...
            print(n_finetune_epoch,epoch_train_loss)
            optimizer.step()
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset_original = build_or_load_dataset(args, tokenizer, mode='translate_original')
        train_dataset_finetune = build_or_load_dataset(args, tokenizer, mode='translate_finetune')
        # begin training!
        _, _ = train(args, train_dataset, train_dataset_original, train_dataset_finetune, model, tokenizer)
