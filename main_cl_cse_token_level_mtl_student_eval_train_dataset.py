...
def init_args():
    ...
    output_dir = f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-{args.exp_type}-{args.seed}-{args.per_gpu_train_batch_size}-token"
...
def get_multi_teacher_probs(args, dataset, model_class):
    ...
    teacher_paths = [f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-teacher_{l}-{args.seed}-32-token/checkpoint" for l
                     in teacher]
...
def main():
    ...
    if args.do_distill:
        ...
        # MTL version of distillation
            ...
            teacher_model_path = f"outputs/{args.tfm_type}-{args.src_lang}-fr-acs_mtl-{args.seed}-{args.per_gpu_train_batch_size}-token/checkpoint"
            # teacher_model_path = args.trained_teacher_paths
            teacher_probs = get_teacher_probs(args, dataset, model_class, teacher_model_path)
        ...
        student_model_path = f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-mtl-{args.seed}-32-token/checkpoint"
    ...
    if args.do_eval:
        ...
        if 'mtl' in exp_type or 'macs_kd' in exp_type:
            one_tgt_lang = 'fr'
            saved_model_dir = f"outputs/{args.tfm_type}-{args.src_lang}-{one_tgt_lang}-{exp_type}-{args.seed}-{args.per_gpu_train_batch_size}-token"
        ...
        log_file_path = f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-{args.exp_type}-{args.seed}-{args.per_gpu_train_batch_size}-token/log.txt"
