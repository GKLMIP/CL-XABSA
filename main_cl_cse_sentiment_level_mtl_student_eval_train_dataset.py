...
def init_args():
    ...
    output_dir = f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-{args.exp_type}-{args.seed}-{args.per_gpu_train_batch_size}-sentiment"
...
class SupConLoss(torch.nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        # print([i[0] for i in labels.tolist()])
        # print(mask)
...
class SupConLoss_logit(torch.nn.Module):
    """Supervised Contrastive Learning: https://arxiv.org/pdf/2004.11362.pdf.
    It also supports the unsupervised contrastive loss in SimCLR"""

    def __init__(self, temperature=0.07, contrast_mode='all',
                 base_temperature=0.07):
        super(SupConLoss_logit, self).__init__()
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        Args:
            features: hidden vector of shape [bsz, n_views, ...].
            labels: ground truth of shape [bsz, label_num].
            mask: contrastive mask of shape [bsz, bsz], mask_{i,j}=1 if sample j
                has the same class as sample i. Can be asymmetric.
        Returns:
            A loss scalar.
        ...
        logits_sim = torch.matmul(labels,labels.T)
        zero = torch.zeros_like(logits_sim)
        one = torch.ones_like(logits_sim)

        logits_sim = torch.where(logits_sim > 0.5, one, zero)
        mask = logits_sim.float().to(device)
        ...
        # anchor_count 1568
        # anchor_feature [1568, 13]   16 * 98 = 1568
...
def train(args, train_dataset, model, tokenizer):
    ...
    for n_epoch, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            hidden_states = ouputs[1]
            hidden_states = hidden_states.view(-1,1,hidden_states.shape[2])

            # loss = ouputs[0]  # model outputs are always tuple in pytorch-transformers (see doc)
            loss = loss(ouputs[1].view(-1,ouputs[1].shape[2]),batch['labels'].to(args.device).view(-1))
            loss_cl = SupConLoss(contrast_mode='all', temperature=0.9).to('cuda')
            loss_cl = loss_cl(hidden_states, batch['labels'].to(args.device).view(-1))
            loss_cl = loss_cl / len(hidden_states)
            loss = 0.5 * loss + loss_cl * 0.5
...
def train_kd(args, train_dataset, model, tokenizer):
    ...
    for n_epoch, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss_cl = SupConLoss_logit(contrast_mode='all', temperature=0.9,base_temperature=0.1).to('cuda')
            loss_cl = loss_cl(hidden_states, batch['teacher_probs'].to(args.device).view(-1, 13))
...
def evaluate(args, eval_dataset, model, idx2tag, mode, step=None):
    ...
    if mode == 'test':
        ...
        file_name_to_write = f'{args.saved_model_dir}/train.pickle'
...
def get_multi_teacher_probs(args, dataset, model_class):
    ...
    teacher_paths = [f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-teacher_{l}-{args.seed}-64-sentiment/checkpoint" for l
                     in teacher]
...
def main():
    ...
    if args.do_distill:
        ...
        # MTL version of distillation
            ...
            teacher_model_path = f"outputs/{args.tfm_type}-{args.src_lang}-fr-acs_mtl-{args.seed}-{args.per_gpu_train_batch_size}-sentiment/checkpoint"
            # teacher_model_path = args.trained_teacher_paths
            teacher_probs = get_teacher_probs(args, dataset, model_class, teacher_model_path)

        teacher_probs_file = f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-{args.exp_type}-{args.seed}-{args.per_gpu_train_batch_size}-sentiment/teacher_probs.json"
        ...
        student_model_path = f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-mtl-{args.seed}-64-sentiment/checkpoint"
    ...
    if args.do_eval:
        ...
        if 'mtl' in exp_type  or 'macs_kd' in exp_type:
            one_tgt_lang = 'fr'
            saved_model_dir = f"outputs/{args.tfm_type}-{args.src_lang}-{one_tgt_lang}-{exp_type}-{args.seed}-{args.per_gpu_train_batch_size}-sentiment"
        ...
        log_file_path = f"outputs/{args.tfm_type}-{args.src_lang}-{args.tgt_lang}-{args.exp_type}-{args.seed}-{args.per_gpu_train_batch_size}-sentiment/log.txt"
